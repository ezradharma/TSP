...
export default function TargetRatePage () {
	...
	useEffect(() => {
        fetch("/rates/target").then(res => res.json())
		.then(data => {
			setSpinner(true)
			setArr({
        ...
        info.push(<a href={data.links[i]}>
            <div>
                ...
                <span>Best Rates: {data.top5_percents[i]}%</span>
                    <h2><FaGreaterThan style={{fontSize: '50px', float: 'right', marginTop: '-90px', color: "blue"}}/></h2>
            </div>
            </a>)
    }
		...
		<div className='SearchWalmart_SearchWalmart'>
			...
			{!spinner ? (
				...
				<div style={{marginTop: '100px', color: "white"}}>
					...
					<br></br>
					<span>Highest Cashback Rates for Target: </span>
